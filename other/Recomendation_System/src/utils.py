...
def pre_filter_items(data_all, item_features, purchases_weeks=52, take_n_popular=5000):
    ...
    # print(data_all['user_id'].unique() - data['user_id'].unique())
    out_users = list(set(data_all['user_id'].tolist()) - set(data['user_id'].tolist()))

    return data, out_users
...
def get_users_features(user_data, train_data, users_embeddings):
    ...
    user_data = user_data[['user_id', 'marital_status_code', 'homeowner_desc', 'hh_comp_desc',
                          'age_desc_int', 'kid_category_desc_int']]  # 'income_desc_int', 'household_size_desc_int',
    ...
    user_data_extend = user_data.merge(users_embeddings, on='user_id', how='left')

    # Делаем фичи типа object категориальными
    cat_feats = ['marital_status_code', 'homeowner_desc', 'hh_comp_desc']
    user_data_extend[cat_feats] = user_data_extend[cat_feats].astype('category')

    return user_data_extend, user_data.iloc[:, (len(cat_feats) + 1)::]
...
def get_items_features(item_data, train_data, items_embeddings):
    ...
    # Эмбеддинги
    item_data_extend = item_data.merge(items_embeddings, on='item_id', how='left')

    return item_data_extend, item_data.iloc[:, (len(cat_feats) + 1)::]
